...
USERS_FILE = 'e:/Profile matching/users.csv'
FEEDBACK_FILE = 'e:/Profile matching/feedback.csv'
CACHE_FILE = 'e:/Profile matching/pipeline_cache.pkl'
...
# Serve frontend
app.mount("/static", StaticFiles(directory="e:/Profile matching/static"), name="static")

@app.get("/")
def index():
    return FileResponse("e:/Profile matching/static/index.html")
...
def get_matches(user_id: str, limit: int = 5):
    ...
    interacted_users = engine.pipeline.feedback_df[engine.pipeline.feedback_df['user_id'] == user_id]['matched_user_id'].tolist()
